            ...
            <div className='mx-auto max-w-[1250px]'>
                <div className='flex justify-between h-[45px]'>
                    ...
                    <div className='flex items-center p-4 bg-[#2A2A2C] rounded-lg cursor-pointer'>
                        ...
                        <p className='font-inter font-normal text-white text-[15px]'>Sign in with VMSC</p>
                    </div>
                </div>
            </div>
        </div>
...
const Header2: React.FC<any> = ({ searchs, setSearch, accessAlert, setAccessAlert, updateTeacherStatus  }) => {
    ...
    useEffect(() => {
        ...
        const getTitle = async () => {
            try{
                ...
                obj.method = 'GET';
                obj.url = 'https://directory-client-server-puce.vercel.app/documents_Title';
        ...
        if(accessAlert === 'timeout'){
            ...
            let interval = setInterval(() => {
                if(count > 0){
                    ...
                    window.location.assign('https://thesisary-delta.vercel.app');
                }
            }, 1000)
        }
    }, [accessAlert])
    ...
    const signIn_Google = async (e: any) => {
        // Check if the email contains 'vmsc'
        const emailParts = e.email.split('@');
        let teacher_true = false;
        let canSignIn = false;

        // Inside signIn_Google


    
        // Check if 'vmsc' is part of the email username (before '@')
        if (emailParts[0].includes('vmsc')) {
            teacher_true = true;
            canSignIn = true;
        }
        // Check if 'vmsc' is part of the domain (after '@')
        else if (emailParts[1].includes('vmsc')) {
            teacher_true = false;
            canSignIn = true;
        }
    
        // Proceed only if the email meets the criteria
        if (canSignIn) {
            const dataObj = {
                email: e.email,
                fullName: e.name,
            ...
            try {
                obj.method = 'POST';
                obj.url = 'https://directory-client-server-puce.vercel.app/signIn';
                obj.data = dataObj;
            ...
            alert('Sign in not allowed. Email must be from VMSC');
        }
    }
                ...
                <div className='flex justify-between lg:h-[45px] md:h-[45px] h-[35px]'>
                    ...
                    {/*____Sign in____*/}
                    {
                        ...
                        <div onClick={() => setSigninClick(true)}
                            ...
                            <p className='font-inter font-normal text-[15px]'>Sign in with VMSC</p>
                        </div>
                        :
                        ''
                    }
            ...
            <div className='fixed w-full h-[100vh] bg-black bg-opacity-20 sm:flex sm:px-5 justify-center items-center signin'>
                <div className='sm:w-[520px] sm:min-h-[200px] w-full min-h-full p-10 rounded-md bg-[#D85900] shadow-lg relative'>
                    ...
                    <div className='mx-auto sm:w-[60%] flex items-center justify-center p-3 my-14 bg-white rounded-lg cursor-pointer overflow-hidden select-none relative'>
                        <img src={ thesisaryWord } alt="thesisaryWord" className='w-6 h-6 mr-3' />
                        <p className='font-inter font-normal text-[15px]'>Continue with VMSC</p>

                        <div className='w-full h-full absolute left-0 top-0 flex justify-center items-center' style={{ 'opacity': '0.01' }}>
                            <GoogleOAuthProvider clientId='294196551117-s4gvis2dkbsnfbqkdfun3kr3qjbl8m0g.apps.googleusercontent.com'>         
                                <GoogleLogin 
            ...
            <div className='w-full h-[100vh] fixed top-0 bg-black bg-opacity-20 header'>
                <div className='w-[60%] h-full float-right bg-[#2A2A2C] menuBurger'>
                    ...
                    <div className='px-6'>
                        ...
                        <p onClick={ () => navigationFun('user') }
                        className='text-white md:text-[18px] text-[14px] flex items-center mb-7 cursor-pointer txtBurger'><img src={ infoA } alt="menu" className='w-6 h-6 mr-3'/> User</p>
                        <div>
                            {/*____Sign in____*/}
                            {
                            !signIns ?
