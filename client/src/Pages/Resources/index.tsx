...
const Resources: React.FC = () => {
    ...
    const rating: Array<string> = ['0', '1', '2', '3', '4', '5'];
    ...
    const [ratingss, setRatings] = useState(false);
    ...
    const btnSearch = async () => {
        ...
        if(searchS.current.value.trim().length === 0 || courseS.current.value === 'Select a Track' || 
            ...
            if(ratingS.current.value === 'Select a Rating'){
                setRatings(true);
            }else{
                setRatings(false);
            }
            ...
            setSearchCache([courseS.current.value, searchS.current.value, yearS.current.value, ratingS.current.value]);
            ...
            try{
                let obj = {
                    method: 'GET',
                    url: 'https://directory-client-server-puce.vercel.app/get_resources',
                    withCredentials: false,
                    params: { course: courseS.current.value, search: searchS.current.value, 
                        year: yearS.current.value, rating: ratingS.current.value, skip: 0 },
                ...
                <div className='mx-auto max-w-[1250px] pt-[115px] pb-[50px]'>
                   ...
                   {/*__Search bar__*/}
                    ...
                    <div className='flex justify-center mt-3'>
                        <div className='lg:w-[60%] w-[100%] grid grid-cols-3'>
                            ...
                            {/*___Year___*/}
                            <select ref={ yearS } className={'w-[99%] text-[15px] p-3 cursor-pointer rounded-md bg-white text-[#767373] '+
                            ...
                            {/*___Rating___*/}
                            <select ref={ ratingS } className={'w-[99%] text-[15px] p-3 cursor-pointer rounded-md bg-white text-[#767373] '+
                            (firstCome ? ratingss ? 'border-2 border-[red]':'':'')} onChange={ () => setSearchFilter('') }>
                                <option selected>Select a Rating</option>
                                { rating.map(a => <option value={a}>{a}</option>) }
                            </select>
                        </div>
                    </div>
                                        ...
                                        <div className='pr-3 mb-3 flex'>
                                            <div key={Math.random()} onClick={() => clickDocu(a._id)}
                                             className='w-full flex-1 rounded-md overflow-hidden bg-[#2A2A2C] cursor-pointer pb-5 shadow-md'>
                                                ...
                                                <p className='lg:text-[17px] md:text-[17px] text-[15px] text-[#FFF200] px-3 line-clamp-3 overflow-hidden'>{ a.title }</p>
                                                <p className='text-[14px] text-white mb-1 px-3'>{ a.year }</p>
                                                <p className='text-[14px] text-white mb-1 px-3'>Member: 0{ a.member }</p>
                                                <p className='text-[15px] text-white mb-3 px-3 '>{ a.course }</p>
                                                <p className='text-[15px] text-white mb-1 px-3 line-clamp-3 overflow-hidden'>{ a.overview }</p>
                                            </div> 
                                        </div>)
                                    }
                                </div>
